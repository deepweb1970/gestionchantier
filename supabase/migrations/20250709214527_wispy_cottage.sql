/*
  ...
  1. New Columns
    - `heure_table` (time) - Stores the lunch break duration to be deducted from total hours

  2. Function Updates
    - Updated `calculate_total_hours()` function to subtract lunch break time from total hours
*/

-- Add heure_table column to saisies_heures table
ALTER TABLE saisies_heures 
ADD COLUMN heure_table time without time zone;
